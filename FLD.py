...
feature_ = ['A','E']
a = df3[df3.label == 1].label.count()
b = df3[df3.label == 0].label.count()
p1 = b/(a+b)
p2 = a/(a+b)
...
def border_of_classifier(sklearn_cl, x, y,n):
        ...
        if(n==1):
            plt.title("FLD分类器训练后的分界线(全部样本)")
            plt.savefig("FLDall")
            
        else:
            plt.title("FLD分类器训练后的分界线(十男十女)")
            plt.savefig("FLD10")
...
def modelparaments():
    models = {}
    models['FLD'] = LinearDiscriminantAnalysis(n_components=9)
    features = {}
