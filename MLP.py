...
def border_of_classifier(sklearn_cl, x, y,n):
        ...
        if(n==1):
            plt.title("MLP分类器训练后的分界线(全部样本)")
            plt.savefig("MLPall")
            
        else:
            plt.title("MLP分类器训练后的分界线(十男十女)")
            plt.savefig("MLP10")
...
def modelparaments():
    models = {}
    models['BP'] = MLPClassifier(solver='lbfgs', alpha=1e-5,
    ...
    df1_all = pd.concat([df1_F10,df1_M10])
    dfs = {}
    dfs['one'] = df1
    dfs['two'] = df1_all
...
def train(model,df1,df2,feature):
    ...
    if(flag ==1):
        ...
        print("测试集上Holdout验证错误率为{},使用的特征为（两个个特征）{},{}".format(1-score,feature[0],feature[1]))
        print("十折交叉验证错误率为{},使用的特征为（两个个特征）{},{}".format(1-score2,feature[0],feature[1]))
        
    else:
        print("训练集上Holdout验证错误率为{},使用的特征为（两个个特征，十男十女）{},{}".format(1-score,feature[0],feature[1]))
